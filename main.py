...
async def execute_command(data: CommandExec):
    ...
    try:
        ...
        elif data.command == "pin":
            # For pin, we use `chat_id` and the `user_id` field actually will hold `message_id` for simplicity
            if not chat_id or not user_id:
                return {"success": False, "error": "Chat ID and Message ID required (use Target User ID field for Message ID)"}
            await bot.app.bot.pin_chat_message(chat_id=chat_id, message_id=user_id, disable_notification=False)
            return {"success": True, "message": f"Message {user_id} pinned in {chat_id}"}
            
        elif data.command == "unpin":
            if not chat_id:
                return {"success": False, "error": "Chat ID required"}
            # If user_id (Message ID) is provided, unpin specific, else unpin all
            if user_id:
                await bot.app.bot.unpin_chat_message(chat_id=chat_id, message_id=user_id)
                return {"success": True, "message": f"Message {user_id} unpinned in {chat_id}"}
            else:
                await bot.app.bot.unpin_all_chat_messages(chat_id=chat_id)
                return {"success": True, "message": f"All messages unpinned in {chat_id}"}
